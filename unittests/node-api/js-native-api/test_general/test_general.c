...
static napi_value testNapiTypeof(napi_env env, napi_callback_info info) {
  ...
  if (argument_type == napi_number) {
    NODE_API_CALL(
        env, napi_create_string_utf8(env, "number", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_string) {
    NODE_API_CALL(
        env, napi_create_string_utf8(env, "string", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_function) {
    NODE_API_CALL(
        env,
        napi_create_string_utf8(env, "function", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_object) {
    NODE_API_CALL(
        env, napi_create_string_utf8(env, "object", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_boolean) {
    NODE_API_CALL(
        env,
        napi_create_string_utf8(env, "boolean", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_undefined) {
    NODE_API_CALL(
        env,
        napi_create_string_utf8(env, "undefined", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_symbol) {
    NODE_API_CALL(
        env, napi_create_string_utf8(env, "symbol", NAPI_AUTO_LENGTH, &result));
  } else if (argument_type == napi_null) {
    NODE_API_CALL(
        env, napi_create_string_utf8(env, "null", NAPI_AUTO_LENGTH, &result));
  }
  return result;
}
...
static napi_value wrap_first_arg(
  ...
  NODE_API_CALL(
      env,
      napi_get_cb_info(
          const_cast<napi_env>(env), info, &argc, &to_wrap, NULL, NULL));
  NODE_API_CALL(
      env,
      napi_wrap(
          const_cast<napi_env>(env), to_wrap, data, finalizer, NULL, NULL));
...
static napi_value testAdjustExternalMemory(
    napi_env env,
    napi_callback_info info) {
  napi_value result;
...
static void
finalizer_only_callback(node_api_basic_env env, void *data, void *hint) {
  napi_ref js_cb_ref = (napi_ref)data;
  napi_value js_cb, undefined;
  NODE_API_CALL_RETURN_VOID(
      env,
      napi_get_reference_value(const_cast<napi_env>(env), js_cb_ref, &js_cb));
  NODE_API_CALL_RETURN_VOID(
      env, napi_get_undefined(const_cast<napi_env>(env), &undefined));
  NODE_API_CALL_RETURN_VOID(
      env,
      napi_call_function(
          const_cast<napi_env>(env), undefined, js_cb, 0, NULL, NULL));
  NODE_API_CALL_RETURN_VOID(
      env, napi_delete_reference(const_cast<napi_env>(env), js_cb_ref));
}
...
static void
cleanup_env_finalizer(node_api_basic_env env, void *data, void *hint) {
  (void)env;
  (void)hint;

  printf(
      "finalize at env cleanup for %s\n",
