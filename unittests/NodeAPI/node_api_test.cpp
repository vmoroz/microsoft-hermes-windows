...
NodeApiTestException::NodeApiTestException(
    napi_env env,
...
void NodeApiTestException::ApplyScriptErrorData(
    napi_env env,
    napi_value error) {
...
/*static*/ napi_value NodeApiTestException::GetProperty(
    napi_env env,
    napi_value obj,
    char const *name) {
  napi_value result{};
...
/*static*/ int32_t NodeApiTestException::GetPropertyInt32(
    napi_env env,
    napi_value obj,
    char const *name) {
  napi_value napiValue = GetProperty(env, obj, name);
...
/*static*/ std::string NodeApiTestException::CoerceToString(
    napi_env env,
    napi_value value) {
  napi_value strValue;
...
/*static*/ std::string NodeApiTestException::ToString(
    napi_env env,
    napi_value value) {
  size_t valueSize{};
...
NodeApiTestErrorHandler NodeApiTest::ExecuteNodeApi(
  ...
  try {
    ...
    return NodeApiTestErrorHandler(nullptr, std::exception_ptr(), "", "", 0, 0);
  } catch (...) {
...
napi_value NodeApiTestContext::RunScript(
...
std::string NodeApiTestContext::ReadScriptText(
    std::string const &testJSPath,
    std::string const &scriptFile) {
...
std::string NodeApiTestContext::ProcessStack(
...
INSTANTIATE_TEST_CASE_P(
    NodeApiEnvs,
    NodeApiTest,
    ::testing::ValuesIn(NodeApiEnvFactories()));
