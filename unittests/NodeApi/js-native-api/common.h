...
#define GET_AND_THROW_LAST_ERROR(env)                                   \
  do {                                                                  \
    ...
    napi_get_last_error_info(const_cast<napi_env>(env), &error_info);   \
    bool is_pending;                                                    \
    napi_is_exception_pending(const_cast<napi_env>(env), &is_pending);  \
    /* If an exception is already pending, don't rethrow it */          \
    if (!is_pending) {                                                  \
      const char *error_message = error_info->error_message != NULL     \
          ? error_info->error_message                                   \
          : "empty error message";                                      \
      napi_throw_error(const_cast<napi_env>(env), NULL, error_message); \
    }                                                                   \
  } while (0)
