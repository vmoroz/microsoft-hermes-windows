...
static void TestFinalizer(node_api_basic_env env, void *raw_data, void *hint) {
  (void)raw_data;
  (void)hint;

  AddonData *data;
  NODE_API_CALL_RETURN_VOID(
      env, napi_get_instance_data(const_cast<napi_env>(env), (void **)&data));
  napi_value js_cb, undefined;
  NODE_API_CALL_RETURN_VOID(
      env,
      napi_get_reference_value(
          const_cast<napi_env>(env), data->js_cb_ref, &js_cb));
  NODE_API_CALL_RETURN_VOID(
  ...
  NODE_API_CALL_RETURN_VOID(
      env, napi_delete_reference(const_cast<napi_env>(env), data->js_cb_ref));
  data->js_cb_ref = NULL;
}
