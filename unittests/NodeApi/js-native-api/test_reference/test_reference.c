...
static napi_value CreateExternalWithFinalize(
    napi_env env,
...
static napi_value CreateReference(napi_env env, napi_callback_info info) {
  ...
  NODE_API_ASSERT(
      env, test_reference != NULL, "A reference should have been created.");
...
static napi_value DeleteReference(napi_env env, napi_callback_info info) {
  NODE_API_ASSERT(
      env, test_reference != NULL, "A reference must have been created.");
...
static void DeleteBeforeFinalizeFinalizer(
  ...
  napi_value value;
  assert(
      napi_get_reference_value(const_cast<napi_env>(env), *ref, &value) ==
      napi_ok);
  assert(value == NULL);
  napi_delete_reference(const_cast<napi_env>(env), *ref);
  free(ref);
}

static napi_value ValidateDeleteBeforeFinalize(
    napi_env env,
    napi_callback_info info) {
  napi_value wrapObject;
  size_t argc = 1;
  NODE_API_CALL(
      env, napi_get_cb_info(env, info, &argc, &wrapObject, NULL, NULL));
...
napi_value Init(napi_env env, napi_value exports) {
  napi_property_descriptor descriptors[] = {
      ...
      DECLARE_NODE_API_PROPERTY(
          "createExternalWithFinalize", CreateExternalWithFinalize),
