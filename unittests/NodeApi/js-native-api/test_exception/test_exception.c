...
static void finalizer(node_api_basic_env env, void *data, void *hint) {
  NODE_API_CALL_RETURN_VOID(
      env,
      napi_throw_error(
          const_cast<napi_env>(env), NULL, "Error during Finalize"));
}
...
static napi_value createExternal(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(
      env, napi_create_external(env, NULL, finalizer, NULL, &external));
